...
export default async function LolDashboardPage() {
  ...
  const [
    ...
    supabase.from('scrim_posts').select('id, game_type, preferred_date, note, server, status, teams(name, tier_avg)').eq('status', 'open').eq('game_type', GAME).order('created_at', { ascending: false }).limit(8),
    ...
    <div className="min-h-screen bg-[#0d0d14]">
      ...
      <div className="pt-28 max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-white/5">
          <span className="text-xs font-black uppercase tracking-widest" style={{ color: GAME_COLOR }}>LEAGUE OF LEGENDS</span>
          <span className="text-slate-700">|</span>
          <span className="text-slate-500 text-xs">LoL 대시보드</span>
        </div>
        ...
        <div className="flex gap-6">
          ...
          <aside className="w-64 shrink-0 flex flex-col gap-3">
            <ProfileCard
              ...
              valGamename={profile?.val_gamename ?? null}
              valTagline={profile?.val_tagline ?? null}
              valTier={profile?.val_tier ?? null}
              lolGamename={profile?.lol_gamename ?? profile?.riot_gamename ?? null}
              lolTagline={profile?.lol_tagline ?? profile?.riot_tagline ?? null}
                    ...
                    {recentMatches.map((m: any) => {
                      ...
                      return (
                        <a key={m.id} href={`/matches/${m.id}`} className="flex items-center gap-3 px-4 py-3 hover:bg-white/3 transition group">
                          ...
                          <span className="text-slate-700 group-hover:text-[#c89b3c] text-xs transition">→</span>
                        </a>
                      )
                    })}
                  </div>
