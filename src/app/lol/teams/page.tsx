...
export default async function LolTeamsPage() {
  ...
  const myLolTeams = (myTeams ?? []).filter((m: any) => m.teams?.game_type === 'lol')
  const myTeamIds = new Set(myLolTeams.map((m: any) => m.teams?.id).filter(Boolean))
      ...
      <div className="pt-28 max-w-5xl mx-auto px-6 py-8">
        ...
        {myLolTeams.length > 0 && (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
              {myLolTeams.map((m: any) => {
                const team = m.teams
                if (!team) return null
                return (
                  <div key={team.id} className="bg-[#13131f] border border-[#c89b3c]/20 rounded p-5 flex items-center justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs bg-[#c89b3c]/20 text-[#c89b3c] px-2 py-0.5 rounded">
        ...
        <div className="bg-[#13131f] border border-white/5 rounded overflow-hidden">
          ...
          {allTeams && allTeams.length > 0 ? (
            <div className="divide-y divide-white/5">
              {allTeams.map((team: any) => (
                <div key={team.id} className="grid grid-cols-12 gap-2 px-5 py-3.5 items-center hover:bg-white/3 transition">
                  <a href={`/teams/${team.id}`} className="col-span-5 text-white font-semibold text-sm hover:text-[#c89b3c] transition">{team.name}</a>
                  <span className="col-span-4 text-slate-400 text-xs">{team.tier_avg ?? '—'}</span>
                  <div className="col-span-3 flex justify-end">
                    {myTeamIds.has(team.id) ? (
                      <span className="text-xs text-[#c89b3c]">소속 중</span>
            ...
            <div className="text-center py-12 text-slate-600 text-sm">등록된 LoL 팀이 없어요</div>
          )}
        </div>
      </div>
    </div>
