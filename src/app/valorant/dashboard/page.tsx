...
export default async function ValorantDashboardPage() {
  ...
  const [
    ...
    supabase.from('team_members').select('role, teams(id, name, abbreviation, game_type, tier_avg, captain_id, logo_url)').eq('user_id', user.id),
            ...
            <div className="bg-[#0d0d1a] border border-white/[0.06] rounded-2xl p-5 card-glow transition-all duration-300">
              ...
              {team ? (
                <a href={`/teams/${team.id}`} className="flex items-center gap-3 group">
                  <div className="relative w-12 h-12 shrink-0 rounded-xl overflow-hidden border border-white/[0.08]">
                    {team.logo_url ? (
                      <img src={team.logo_url} alt="팀 로고" className="w-full h-full object-cover" />
                    ) : (
                    ...
                    {myValTeam?.role === 'captain' && !team.logo_url && (
                      <a href={`/teams/${team.id}/manage`}
                        ...
                        <span className="text-white text-[9px] font-bold leading-none">로고 추가</span>
                      </a>
                    )}
                  </div>
