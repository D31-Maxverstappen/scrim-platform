...
import ReceivedApplications from '@/components/scrim/ReceivedApplications'
...
export default async function ValorantDashboardPage() {
  ...
  const [
    ...
    supabase.from('scrim_posts').select('id, game_type, preferred_date, note, server, format, status, teams(name, tier_avg)').eq('status', 'open').eq('game_type', GAME).order('created_at', { ascending: false }).limit(5),
              ...
              {team ? (
                <div className="flex items-center gap-3">
                  ...
                  <a href={myValTeam?.role === 'captain' ? `/teams/${team.id}/manage` : `/teams/${team.id}`}
                    className="group/logo relative w-12 h-12 shrink-0 rounded overflow-hidden border border-dashed flex items-center justify-center"
                    style={{ borderColor: 'var(--border-input)', background: 'var(--bg-input)' }}>
                    <span className="avatar-upload-icon text-2xl font-light leading-none select-none">+</span>
                  </a>
                  {/* 팀 이름 → 팀 페이지 */}
