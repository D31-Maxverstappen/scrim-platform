    ...
    <div className="min-h-screen bg-[#07070b]">
      ...
      <div className="pt-28 max-w-3xl mx-auto px-6 py-10">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-white font-black text-2xl">
                {profile?.riot_gamename ?? t('profile_default_name', lang)}
              </h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          ...
          {/* 매너 점수 */}
          <div className="bg-[#111118] border border-white/5 rounded p-5">
            <p className="text-slate-500 text-xs uppercase tracking-widest mb-3">{t('profile_manner', lang)}</p>
            ...
            <p className="text-slate-600 text-xs">{t('manner_default', lang)} · 0~200pt</p>
          </div>

          {/* 소속 팀 */}
          <div className="bg-[#111118] border border-white/5 rounded p-5">
            <p className="text-slate-500 text-xs uppercase tracking-widest mb-3">{t('profile_team', lang)}</p>
            {team ? (
              <>
                ...
                <span className="inline-block mt-2 text-xs bg-[#00D2BE]/20 text-[#00D2BE] px-2 py-0.5 rounded">
                  {teamMember?.role === 'captain' ? t('my_team_captain', lang) : t('my_team_member', lang)}
                </span>
              </>
            ) : (
              <div className="flex flex-col gap-2">
                <p className="text-slate-600 text-sm">{t('profile_no_team', lang)}</p>
                <a href="/teams/create" className="text-xs text-[#00D2BE] hover:underline">{t('my_team_create', lang)} →</a>
              </div>
            )}
          </div>
          ...
          {/* 전적 */}
          <div className="bg-[#111118] border border-white/5 rounded p-5">
            <p className="text-slate-500 text-xs uppercase tracking-widest mb-3">{t('profile_scrim_record', lang)}</p>
            <div className="flex items-end gap-2 mb-1">
              <span className="text-4xl font-black text-white">0</span>
              <span className="text-slate-500 text-sm mb-1">{t('win', lang)}</span>
              <span className="text-4xl font-black text-slate-600">0</span>
              <span className="text-slate-600 text-sm mb-1">{t('loss', lang)}</span>
            </div>
            <p className="text-slate-600 text-xs">{t('team_winrate', lang)} —</p>
          </div>
        ...
        <div className="bg-[#111118] border border-white/5 rounded overflow-hidden mb-6">
          <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between">
            <h2 className="text-white font-bold text-sm">{t('profile_recent_scrims', lang)}</h2>
            <a href="/scrims" className="text-[#00D2BE] text-xs hover:underline">{t('view_all', lang)}</a>
          </div>
          <div className="flex flex-col items-center justify-center py-16 text-slate-600">
            ...
            <p className="text-sm">{t('profile_no_scrims', lang)}</p>
          </div>
        </div>
        ...
        <div className="border border-red-500/20 bg-red-500/5 rounded p-6 flex items-center justify-between">
          <div>
            <p className="text-white font-semibold text-sm">{t('profile_delete_acct', lang)}</p>
