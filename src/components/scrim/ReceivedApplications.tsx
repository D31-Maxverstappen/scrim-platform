  ...
  return (
    <div className="bg-[#0d0d1a] border border-white/[0.06] rounded-2xl overflow-hidden card-glow transition-all duration-300">
      <div className="px-5 py-4 border-b border-white/[0.04] flex items-center gap-3">
        <p className="text-[11px] font-black uppercase tracking-[0.15em] text-slate-600">받은 스크림 신청</p>
        {pending.length > 0 && (
          <span className="text-[11px] font-black bg-[#ff4655] text-white px-2 py-0.5 rounded">
            {pending.length}
          </span>
        )}
      </div>
            ...
            <div key={app.id} className="px-5 py-4 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  {app.applying_team.tier_avg && (
                    <span className="text-slate-600 text-[11px]">{app.applying_team.tier_avg}</span>
                  )}
                </div>
                <p className="text-slate-600 text-[11px]">희망 시간: {date}</p>
                {app.scrim_post.note && (
                  <p className="text-slate-700 text-[11px] truncate mt-0.5">{app.scrim_post.note}</p>
                )}
              </div>
              ...
              <div className="shrink-0 flex items-center gap-2">
                {app.status === 'pending' ? (
                  accepting === app.id ? (
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        disabled={!!loading}
                        className="text-[11px] font-bold px-3 py-1.5 rounded-lg bg-[#00D2BE]/15 text-[#00D2BE] border border-[#00D2BE]/30 hover:bg-[#00D2BE]/25 transition disabled:opacity-50"
                      >
                        {loading === app.id + 'accept' ? '...' : '확정'}
                      </button>
                      <button onClick={() => setAccepting(null)} className="text-[11px] text-slate-600 hover:text-slate-400 transition">✕</button>
                    </div>
                  ) : (
                    <>
                      <button
                      ...
                      <button
                        ...
                        disabled={!!loading}
                        className="text-[11px] font-bold px-3 py-1.5 rounded-lg bg-white/[0.04] text-slate-500 border border-white/[0.06] hover:bg-white/[0.08] transition disabled:opacity-50"
                      >
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] font-bold text-green-400">수락됨</span>
                    {matchLinks[app.id] && (
                      <a href={`/matches/${matchLinks[app.id]}`}
                        className="text-[11px] font-bold px-3 py-1.5 rounded-lg bg-[#00D2BE]/15 text-[#00D2BE] border border-[#00D2BE]/30 hover:bg-[#00D2BE]/25 transition">
                        매치 보기 →
                      </a>
                    )}
                  </div>
                ) : (
                  <span className="text-[11px] font-bold text-slate-700">거절됨</span>
                )}
              </div>
            </div>
          )
